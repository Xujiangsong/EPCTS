...
class Attention(Layer):
    def __init__(self, op='attsum', activation='tanh', init_stdev=0.01, **kwargs):   #当 op='attsum' 时，表示进行加权求和操作
        ...
        super(Attention, self).__init__(**kwargs)   #通过调用父类的初始化函数，可以保证 Attention 类继承了 Layer 类的一些基本行为和特性。

    def build(self, input_shape):    #根据输入数据的形状构建注意力层的权重。
